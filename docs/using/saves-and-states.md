...
Same flow as saves but under **Upload State**, with an optional screenshot attachment. One is autogenerated when you create a state from in-browser play, so the manual screenshot only matters when uploading from outside.
...
**Game Data → Saves/States → Context menu → Delete** removes both the DB row and the file, and can't be undone.
