...
The file's stored under `/romm/assets/<user>/<rom>/saves/` and is available to your [in-browser play](in-browser-play/emulatorjs.md) sessions. File contents aren't rewritten. It's stored as-is.
...
- Screenshots can be attached. One is autogenerated from the emulator if you create a state from [in-browser play](in-browser-play/emulatorjs.md).
