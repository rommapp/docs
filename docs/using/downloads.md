...
Context menu (…) on a game card → **Copy download link** → URL is on your clipboard.
...
Context menu (…) → **Show QR Code** → point the other device's camera at the screen.
...
Multi-select ROMs in any gallery (platform view, collection, search results) → toolbar → **Download selected** → single zip with every selected ROM preserved in its platform folder structure.
