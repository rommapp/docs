...
description: Auto-generated groupings by genre, developer, year, tag — no rules to write.
---
...
Collections with too few ROMs are suppressed — you won't see a "Developer: Some Indie Studio" with one entry.
...
Disabling a dimension hides all its collections from your drawer and dashboard. The underlying data is still there; you can re-enable any time.
...
- Remove a ROM (its metadata determines membership — fix the metadata if it's wrong).
...
- **ScreenScraper** — ok genre data; some provider-specific labels.
- **Hasheous / PlayMatch** (hash-only) — no genre data; they proxy IGDB.
